...
export function HeadlessNotificationsCenter({
  ...
  const selected = useMemo(() => {
    ...
    return (
      notifications.find((n) => getNotificationId(n) === selectedId) ?? null
    );
                ...
                <div className="flex flex-col">
                  {loading ? (
                    ...
                    notifications.map((n, idx) => {
                      ...
                      const selected = selectedId === actionId;
                      ...
                      return (
                        <button
                          ...
                          onClick={() => setSelectedId(actionId ?? id)}
                    ...
                    {getNotificationId(selected) ? (
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => markRead(getNotificationId(selected)!)}
                        >
                          Mark read
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => archive(getNotificationId(selected)!)}
                        >
                          Archive
                        </Button>
                      </div>
