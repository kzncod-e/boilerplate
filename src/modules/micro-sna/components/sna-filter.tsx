        ...
        <form className="grid grid-cols-2 w-full gap-6 mt-4">
          ...
          <FormField className="w-full">
            ...
            <Select
              ...
              onValueChange={(v) => {
                const found = TOPICS.find((t) => String(t.id) === v);
                setTopic(found ?? null);
              }}
            >
          ...
          <div className="w-full">
            ...
            <Popover>
              ...
              <PopoverContent className="w-full">
                {["facebook", "twitter", "instagram", "youtube", "tiktok"].map(
                  (p) => (
                    <div
                      key={p}
                      className="flex items-center justify-between py-1 cursor-pointer"
                      onClick={() =>
                        toggle(p as SnaPlatform, platforms, setPlatforms)
                      }
                      role="button"
                      tabIndex={0}
                      onKeyDown={(e) =>
                    ...
                    >
                      ...
                      <span className="ml-2">
                        {platforms.includes(p as SnaPlatform) ? (
          ...
          {/* EMOTIONAL */}
          <div className="w-full">
            <p className="text-sm font-medium">Emotional Perception</p>
            <Select
              value={sentimentSelected}
              onValueChange={(v) =>
                setSentimentSelected(v as "linguistik" | "prokontra")
              }
            >
              <SelectTrigger className="w-full">
                <SelectValue />
              ...
              <SelectContent className="w-full">
                <SelectItem value="linguistik">Linguistik</SelectItem>
                <SelectItem value="prokontra">Prokontra</SelectItem>
              </SelectContent>
            </Select>
          </div>
                  ...
                  >
                    ...
                    <span className="ml-2">
                      {(sentimentSelected === "prokontra"
                        ? prokontra
                        : sentiments
                      ).includes(s.val as SentimentNumber) ? (
                        <Check className="w-4 h-4 text-blue-600" />
                      ) : (
                        <span className="w-4 h-4" />
                      )}
